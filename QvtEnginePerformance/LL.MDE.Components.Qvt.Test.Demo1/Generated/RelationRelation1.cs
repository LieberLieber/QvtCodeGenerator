namespace LL.MDE.Components.Qvt.Transformation.Demo1
{
	...
	public class RelationRelation1
	{
		...
		{
			ISet<CheckResultRelation1> result = new HashSet<CheckResultRelation1>();ISet<MatchDomainP> matchDomainPs = CheckDomainP(p);ISet<MatchDomainP2> matchDomainP2s = CheckDomainP2(p2);foreach (MatchDomainP matchDomainP in matchDomainPs ) {foreach (MatchDomainP2 matchDomainP2 in matchDomainP2s ) {string s = matchDomainP.s;
			...
			if(p2.Name == (s + "Other")) {CheckResultRelation1 checkonlysMatch = new CheckResultRelation1 () {matchDomainP = matchDomainP,matchDomainP2 = matchDomainP2,};
		...
		internal MatchDomainPo EnforcePo(string s,string someString,int source, LL.MDE.DataModels.EnAr.Package po)
		{
			MatchDomainPo match = new MatchDomainPo();
			...
			if (e== null) {
			e = po.Elements.OfType<LL.MDE.DataModels.EnAr.Element>().FirstOrDefault(var128344654 => var128344654?.Name == s + someString);
